...
- **电影级机位与平滑运镜**：提供 6 大预设视角（前45°、正侧、正前、后45°、轮毂特写、俯视），支持 `1~6 / R` 快捷键与平滑缓动运镜
- **改前 vs 改后 OEM 对比**：一键切换个性化改装方案与原厂出厂素车（原厂素白/无拉花/原厂轮毂/原厂卡钳），支持 `C` 键随时对比
- **高清改装海报离屏生成**：一键合成 1200×1600 赛博暗黑机能风海报，包含 3D 渲染画面、6 格配置清单、总报价、门店认证与扫码交互二维码
- **4 大展厅光照场景**：极简暗调影棚、赛博霓虹夜景、日落黄金余晖、户外竞技赛道，支持 `E` 键循环切换
- **沉浸式改装工坊音效**：纯前端 Web Audio 合成喷漆气流、运镜风阻、机械锁紧、相机快门声效，支持 `M` 键快速静音
- **自定义车型导入**：上传已打包纹理的 `.glb`，自动识别车漆 / 玻璃 / 轮毂 / 卡钳材质，支持手动勾选映射后保存
- **完整客户流程**：配置方案 → 自动报价 → 离屏海报/分享 → 预约到店
- **门店管理后台**：查看预约、订单金额与客户方案，支持确认和取消
- **桌面原生客户端**：基于 Electron 打包 macOS Apple Silicon 原生 App，支持离线运行与本地数据持久化
...
- **SHA-256 校验和**：`ebe47ab2141b359344a984109edf120b8dfe1f47356f45dc4363c7678ae4b032`
